...
Recorded on 2026-06-09 while experimenting with `propagators.tui.graph.vijual` and
...
- a compiler graph should also have a collapsed semantic view that hides
  mechanical compiler cells and propagators.infra.
...
```bash
clojure -M -m propagators.tui.graph.vijual-compiler-2-demo
```

The demo draws two kinds of propagators.tui.graph.
...
```bash
clojure -M -m propagators.tui.graph.vijual-clojure-lang-demo
```
