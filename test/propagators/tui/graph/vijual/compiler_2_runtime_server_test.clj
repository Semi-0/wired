(ns propagators.tui.graph.vijual.compiler-2-runtime-server-test
  (:require [charm.components.text-input :as text-input]
            ...
            [propagators.tui.graph.xr-server :as xr-server]
...
(deftest server-transport-normalizes-cell-records
  (let [transported (#'server/transport-value
    ...
    (is (not (str/includes? (pr-str transported)
                            "propagators.infra.cells.cell.Cell")))
    (is (not (str/includes? (pr-str transported)
                            "propagators.infra.network.Net")))))
...
(deftest tui-annotations-compact-trace-and-derived-event-identities
  (let [node-id (ids/new-node-id)
    ...
    (is (not (str/includes? trace-label "#propagators.infra.ids.NodeId")))
    (is (not (str/includes? trace-label ":trace/subscribe")))
    (is (not (str/includes? event-label "#propagators.infra.ids.NodeId")))))
